...
def generate_embeddings(texts):
    ...
    try:
        embeddings = []
        for text in texts:
            # Using Groq to generate embeddings
            response = groq_client.chat.completions.create(
                messages=[
                    {"role": "system", "content": "You are an embedding generator. Your task is to generate a numerical representation of the input text. Return only the embedding as a JSON array."},
                    {"role": "user", "content": f"Generate an embedding representation for the following text: {text}"}
                ],
                model="llama3-8b-8192",  # Choose the appropriate model for embeddings
                response_format={"type": "json_object"},
                temperature=0.0
            )
            # Parse the embedding from the response
            embedding_json = json.loads(response.choices[0].message.content)
            # Ensure the JSON has a key 'embedding' where the vector is stored
            if 'embedding' in embedding_json:
                embedding = embedding_json['embedding']
                # Check if we need to resize the embedding to match the expected dimension
                if len(embedding) != dimension:
                    # Pad or truncate the embedding to match the expected dimension
                    if len(embedding) < dimension:
