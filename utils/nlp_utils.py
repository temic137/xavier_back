...
# Add these functions to your existing nlp_utils.py file

def get_relevant_context_for_chatbot(chatbot, question, max_passages=5):
    """
    Retrieve only the relevant context passages for a given question.
    This is much more memory-efficient than loading all context data.
    
    Args:
        chatbot: The Chatbot model instance
    ...
    """
    try:
        # Parse the chatbot data
        chatbot_data = parse_chatbot_data(chatbot.data)
        
        # Extract all text content from the chatbot data
        all_passages = extract_text_passages(chatbot_data)
        
        # Get the most relevant passages using BM25 retrieval
        relevant_passages = get_relevant_passages(question, all_passages, top_k=max_passages)
        
        logging.debug(f"Retrieved {len(relevant_passages)} relevant passages")
        print(f"Retrieved {len(relevant_passages)} relevant passages")
        return relevant_passages
        
    except Exception as e:
        logging.error(f"Error retrieving context: {str(e)}")
        # Return empty list on error to avoid breaking the whole flow
        return []

def parse_chatbot_data(data):
    """Parse the chatbot data safely from various formats"""
    if isinstance(data, str):
        try:
            return json.loads(data)
        except json.JSONDecodeError:
            logging.error("Failed to parse chatbot data JSON string")
            return {}
...
def extract_text_passages(data, max_length=500):
    """
    Extract text passages from the chatbot data.
    Processes the data in chunks to minimize memory usage.
    
    Args:
        data: Parsed chatbot data (dict or list)
    ...
    if isinstance(data, list):
        # Take only the latest data if it's a list
        if data:
            data = data[-1]
        else:
            return passages
    ...
    for item in pdf_data:
        if isinstance(item, dict) and 'text' in item:
            ...
            if passage_text:
                ...
                chunks = split_text_into_chunks(passage_text, max_length)
    ...
    if isinstance(web_data, dict):
        # Extract title
        if 'title' in web_data:
            passages.append(web_data['title'])
        ...
        for section in sections:
            if isinstance(section, dict):
                if 'heading' in section:
                    passages.append(section['heading'])
                if 'content' in section and isinstance(section['content'], list):
                    for content_item in section['content']:
                        if isinstance(content_item, str) and content_item:
                            chunks = split_text_into_chunks(content_item, max_length)
                            passages.extend(chunks)
    
    # Remove duplicates and empty passages
    filtered_passages = [p for p in passages if p and p.strip()]
    unique_passages = list(set(filtered_passages))
    
    return unique_passages

def split_text_into_chunks(text, max_length=500):
    """
    Split a long text into smaller chunks for efficient processing.
    ...
    for para in paragraphs:
        if len(para) <= max_length:
            ...
            try:
                ...
                # Fall back to crude splitting if NLTK fails
                logging.warning(f"NLTK sentence tokenization failed: {str(e)}")
                for i in range(0, len(para), max_length):
                    chunks.append(para[i:i + max_length])
    
    return chunks

def extract_keywords(question, min_length=4):
    """Extract relevant keywords from the question"""
    # Simple keyword extraction
    words = question.lower().split()
    # Filter by length and remove common words
    common_words = {'what', 'where', 'when', 'who', 'why', 'how', 'is', 'are', 'the', 'and', 'this', 'that'}
    keywords = [word for word in words if len(word) >= min_length and word not in common_words]
    
    # Add domain-specific keywords if present
    domain_keywords = ["proce", "inventory", "stock", "available", "category", "type"]
    for keyword in domain_keywords:
        if keyword in question.lower() and keyword not in keywords:
            keywords.append(keyword)
    
    return keywords

def track_question_helper(chatbot_id, analytics_data):
    """Helper function to track question analytics"""
    try:
        analytics = QuestionAnalytics(
            chatbot_id=chatbot_id,
            question=analytics_data.get('question', ''),
            answer=analytics_data.get('answer', ''),
            question_metadata=analytics_data.get('question_metadata', {})
        )
        db.session.add(analytics)
        db.session.commit()
    except Exception as e:
        logging.error(f"Error tracking question analytics: {str(e)}")
